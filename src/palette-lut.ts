 ...
 *
 * 性能优化:把"HSL→RGBA + applyTP"从每字符(per-cell)开销转为每帧(per-frame)查表
 * - 静态 LUT(coldStatic / warmStatic):256 项,HSL→RGBA,palette 变化时重建
 * - 终态 LUT(coldFinal / warmFinal):256 项,applyTP 后的 RGBA,TP 或 hue 变化时重建
 * - 每字符颜色用 4 字节 (Uint8Array 1024 bytes/LUT),整张表 < 8KB
 *
 * 失效规则:
 *   setPalettes / setTheme → 静态 LUT 重建,终态 LUT 标记 dirty
 *   setThemeParams / setColdThemeParams / setWarmThemeParams → 终态 LUT dirty
 *   setHueRotate / dynamicHue / colorCurve → 终态 LUT dirty
 ...
 */
export const applyTP = (r: number, g: number, b: number, p: ThemeParams, extraHue: number = 0): [number, number, number] => {
  // brightness

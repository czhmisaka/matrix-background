...
globalThis.HTMLElement = class HTMLElement extends StubElement {
  constructor() { super('html-element'); }
...
test('5 主题 cold/warm 数值范围合法(h ∈ [0,360]、s ∈ [0,1]、lMin < lMax)', () => {
  for (const name of ALL_THEMES) {
    ...
    for (const side of ['cold', 'warm']) {
      const p = t[side];
      assert.ok(typeof p.h === 'number' && Number.isFinite(p.h),
        `${name}.${side}.h 必须是有限 number,实际 ${p.h}`);
      assert.ok(p.h >= 0 && p.h <= 360,
        `${name}.${side}.h 必须在 [0, 360],实际 ${p.h}`);
      assert.ok(typeof p.s === 'number' && Number.isFinite(p.s),
        `${name}.${side}.s 必须是有限 number,实际 ${p.s}`);
      assert.ok(p.s >= 0 && p.s <= 1,
        `${name}.${side}.s 必须在 [0, 1],实际 ${p.s}`);
      assert.ok(typeof p.lMin === 'number' && p.lMin >= 0 && p.lMin <= 1,
        `${name}.${side}.lMin 必须在 [0, 1],实际 ${p.lMin}`);
      assert.ok(typeof p.lMax === 'number' && p.lMax >= 0 && p.lMax <= 1,
        `${name}.${side}.lMax 必须在 [0, 1],实际 ${p.lMax}`);
      assert.ok(p.lMin < p.lMax,
        `${name}.${side}.lMin (${p.lMin}) 必须 < lMax (${p.lMax})`);
    }
  }
...
test('5 主题 TP 7 字段(brightness/chroma/hueShift/saturationShift/lightnessShift/invertHue/contrast)都是有限 number', () => {
  const tpFields = ['brightness', 'chroma', 'hueShift', 'saturationShift', 'lightnessShift', 'invertHue', 'contrast'];
  for (const name of ALL_THEMES) {
    const t = themes[name]();
    for (const f of tpFields) {
      assert.ok(typeof t[f] === 'number' && Number.isFinite(t[f]),
...
test('5 主题 aMax 字段存在且 ∈ [0, 1]', () => {
  for (const name of ALL_THEMES) {
    ...
    for (const side of ['cold', 'warm']) {
      const p = t[side];
      assert.ok(typeof p.aMax === 'number' && p.aMax >= 0 && p.aMax <= 1,
...
test('coldFrom / warmFrom 拼色:主题冷板来自 lava-red,暖板来自 matrix-green', () => {
  ...
  assert.doesNotThrow(() => inst.setTheme('silicon-valley'),
...
test('反复切主题 50 次(10 主题循环)无 throw,实例 destroy 正常', () => {
  const cycle = [...ALL_THEMES, 'silicon-valley', 'matrix-green', 'lava-red', 'cyber-blue', 'pure-mono'];
  const inst = matrixRain({ theme: 'silicon-valley' });
  for (let i = 0; i < 50; i++) {
...
test('冷暖双板色相拉开:冷板 h ∈ [90, 300](蓝绿紫),暖板 h ∈ [0, 60] ∪ [300, 360](红橙黄品红)', () => {
  for (const name of ALL_THEMES) {
    ...
    assert.ok(coldH >= 60 && coldH <= 300,
      `${name}.cold.h (${coldH}) 应在冷色区间 [60°, 300°]`);
    // 暖板:色相应在"暖色"区间(红 0-30 / 橙 30-60 / 黄 60-90 / 品红 300-360)
    const isWarmRange = (warmH <= 90) || (warmH >= 300);
    assert.ok(isWarmRange,
      `${name}.warm.h (${warmH}) 应在暖色区间 [0°,90°] ∪ [300°,360°]`);
  }
...
test('lava-red.warm.h ∈ [0, 50](橙红区)', () => {
  ...
  assert.ok(t.warm.h >= 0 && t.warm.h <= 50,
    `lava-red.warm.h (${t.warm.h}) 应在 [0, 50](橙红)`);
...
test('matrix-green.cold.h ∈ [100, 160](绿区)', () => {
  ...
  assert.ok(t.cold.h >= 100 && t.cold.h <= 160,
...
test('cyber-blue 冷板偏蓝(200-250),silicon-valley 冷板偏青(180-220)', () => {
  ...
  assert.ok(cyber.cold.h >= 200 && cyber.cold.h <= 250,
    `cyber-blue.cold.h (${cyber.cold.h}) 应在蓝区 [200, 250]`);
  assert.ok(sv.cold.h >= 180 && sv.cold.h <= 220,

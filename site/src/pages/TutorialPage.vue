  ...
  <div class="tutorial">
    <div class="container">
      <header class="page-header">
        ...
        <h1>从 0 到生产环境</h1>
        <p class="lead">12 个章节覆盖安装、5 分钟上手、5 主题 × 4 变体预览、3 框架集成 (React / Vue / Next.js)、CDN 兜底、API 完整参考、内置 Playground。</p>
      </header>
      ...
      <section class="tab-content">
        ...
        <article v-if="active === 'intro'">
          ...
          <p>Matrix 风格的数字雨看上去浪漫,实现起来却满是工程权衡。@xietuier/matrix-rain 把 5 主题 4 变体的 Canvas 2D 渲染打包成一个零依赖的小库 — 29 KB gzip,5 行代码即可启动。</p>
          <h3>核心特性</h3>
          <ul>
            <li><b>5 主题预设</b> — 硅谷冷 / 矩阵绿 / 熔岩红 / 赛博蓝 / 纯单色,每个都可冷暖双板拼色</li>
            <li><b>4 变体形态</b> — classic / avalanche / ripple / ascii,字符运动模式各不同</li>
            <li><b>Web Component</b> — <code>&lt;matrix-rain&gt;</code> 标签即可挂载,Vue/React 都能识别</li>
          ...
          <p>DOM 节点是浏览器最擅长的渲染单元,但当屏幕上要出现几百个独立运动的小元素时,DOM 会成为瓶颈。Canvas 2D 的方案简单得多:把所有字符当成像素画在同一个画布上,CPU 端只算逻辑态(位置、亮度、字符),GPU 端只刷一层 texture,几乎零 layout 开销。</p>
        </article>
...
rain.<span class="tk-fn">setTargetBitmap</span>(bitmap, { phase: <span class="tk-st">'noise-converge'</span> });</code></pre>
        ...
        <article v-else-if="active === 'themes'">
          ...
          <p>每个主题都是独立的 HSL 调色板,带 <code>brightness</code> / <code>chroma</code> / <code>hueShift</code> / <code>contrast</code> 等参数。</p>
          <div class="theme-grid">
            <div v-for="t in themeList" :key="t.name" class="theme-card"
...
<span class="tk-tag">&lt;/html&gt;</span></code></pre>
        ...
        <article v-else-if="active === 'api-options'">
          ...
          <table class="api-table">
            ...
            <tbody>
              <tr><td><code>theme</code></td><td><code>ThemeName | &#123; coldFrom, warmFrom &#125;</code></td><td><code>'silicon-valley'</code></td><td>主题预设或冷暖双主题拼色</td></tr>
              <tr><td><code>variant</code></td><td><code>'classic' | 'avalanche' | 'ripple' | 'ascii'</code></td><td><code>'classic'</code></td><td>变体形态</td></tr>
              <tr><td><code>fontSize</code></td><td><code>number</code></td><td><code>14</code></td><td>字符网格宽度(像素)</td></tr>
              <tr><td><code>trailAlpha</code></td><td><code>number</code></td><td><code>0.18</code></td><td>残影 alpha (0-1)。越小拖尾越长</td></tr>
              <tr><td><code>maxDPR</code></td><td><code>number</code></td><td><code>2</code></td><td>DPR 缩放上限,性能优先可设 1</td></tr>
              <tr><td><code>charset</code></td><td><code>string</code></td><td><code>'0123456789'</code></td><td>字符集</td></tr>
              <tr><td><code>targetPhase</code></td><td><code>'fade' | 'noise-converge'</code></td><td><code>'fade'</code></td><td>目标位图出现方式</td></tr>
              <tr><td><code>targetLockOrder</code></td><td><code>'random' | 'topdown' | ... (7 种)</code></td><td><code>'random'</code></td><td>noise-converge 锁定顺序</td></tr>
              <tr><td><code>targetLockStability</code></td><td><code>number</code></td><td><code>0.7</code></td><td>锁定后字符稳定性 (0-1)</td></tr>
              <tr><td><code>onFrame</code></td><td><code>(info: FrameInfo) =&gt; void</code></td><td>—</td><td>帧事件(30Hz 节流)</td></tr>
              <tr><td><code>onResize</code></td><td><code>(size: SizeInfo) =&gt; void</code></td><td>—</td><td>resize 事件(200ms debounce)</td></tr>
              <tr><td><code>onThemeChange</code></td><td><code>(newTheme: ThemeName) =&gt; void</code></td><td>—</td><td>主题切换事件</td></tr>
              <tr><td><code>onTargetFinish</code></td><td><code>() =&gt; void</code></td><td>—</td><td>target 淑出完成事件</td></tr>
            </tbody>
          </table>
        </article>
        ...
        <article v-else-if="active === 'api-methods'">
          ...
          <table class="api-table">
            ...
            <tbody>
              <tr><td><code>destroy()</code></td><td>销毁实例,释放 rAF / 事件监听</td></tr>
              <tr><td><code>pause() / resume()</code></td><td>暂停 / 恢复动画(切 tab 省电)</td></tr>
              <tr><td><code>setTheme(name, opts?)</code></td><td>动态切换主题,opts.keepPaletteParams 保留调参</td></tr>
              <tr><td><code>setPalettes(cold, warm)</code></td><td>动态更新色板(高级)</td></tr>
              <tr><td><code>setDensity(fontSize)</code></td><td>动态调密度</td></tr>
              <tr><td><code>setFlickerSpeed(speed)</code></td><td>动态调闪烁速度倍率(0=冻结, 1=默认, &gt;1=加快)</td></tr>
              <tr><td><code>setTargetFPS(fps)</code></td><td>动态设目标帧率上限(0=不限,30=省电)</td></tr>
              <tr><td><code>setThemeParams(p)</code></td><td>动态调亮度/饱和/色相</td></tr>
              <tr><td><code>setHueRotate(speed, amount?)</code></td><td>热更新时间驱动色相旋转</td></tr>
              <tr><td><code>setColorOverrides(overrides | null)</code></td><td>热更新颜色注入(传 null 清除)</td></tr>
              <tr><td><code>setTargetBitmap(bm, opts?)</code></td><td>设置目标位图 + 计时器自动重置</td></tr>
              <tr><td><code>clearTargetBitmap()</code></td><td>立即淑出(提前结束显示)</td></tr>
              <tr><td><code>getFPS()</code></td><td>读取当前 FPS</td></tr>
              <tr><td><code>getOptions()</code></td><td>读取当前生效配置快照</td></tr>
              <tr><td><code>serialize()</code></td><td>导出 JSON 字符串(SSR hydration / 持久化)</td></tr>
              <tr><td><code>getTargetState?()</code></td><td>读取 noise-converge 状态机当前快照(调试用)</td></tr>
            </tbody>
          </table>
        </article>
...
const themeList = [
  { id: 'silicon-valley', name: 'Silicon Valley',  swatch: '#7af7d4', desc: '硅谷冷光,科技感最强,默认主题' },
  { id: 'matrix-green',   name: 'Matrix Green',    swatch: '#4ade80', desc: '经典黑客帝国绿,最纯粹的数字雨' },
...
.tab-content h2 { margin: 0 0 24px; font-size: clamp(28px, 3.4vw, 40px); }
.tab-content h3 { margin: 32px 0 12px; font-size: 18px; color: var(--text); }
.tab-content p, .tab-content li { font-size: 15px; line-height: 1.7; color: var(--text-muted); }
.tab-content ul { padding-left: 24px; }
.tab-content li { margin-bottom: 8px; }
.tab-content code { font-size: 13px; }
.tab-content b { color: var(--text); font-weight: 500; }
...
.theme-card p { font-size: 12px; margin: 8px 0 0; }
.theme-dot {
...
.variant-card h3 { margin: 0 0 6px; font-family: var(--font-mono); font-size: 14px; }
.variant-card code { display: inline-block; margin-bottom: 10px; }
.variant-card p { font-size: 12px; margin: 0; }
...
.api-table td { color: var(--text); vertical-align: top; }
.api-table tr:last-child td { border-bottom: none; }
.api-table code { font-size: 12px; }

.pg-cta { margin: 16px 0 32px; }
